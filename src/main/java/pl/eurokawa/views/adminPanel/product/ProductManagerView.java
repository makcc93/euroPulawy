...
public class ProductManagerView extends Div {
    ...
    public ProductManagerView(ProductRepository productRepository, TokenService tokenService, SecurityService securityService, EmailService emailService, TokenRepository tokenRepository){
    ...
    private void createActionColumn(Grid<Product> grid,ProductRepository productRepository) {
        grid.addColumn(new ComponentRenderer<>(product -> {
            ...
            delete.addClickListener(deleteClick ->{
                ...
                Token token = tokenService.generateToken(user, TokenType.SIX_NUMBERS);
